...
class shapegenerator {
  constructor(xco,yco,size,color){
  this.xco = xco;
  this.yco = yco;
  this.size = size;
}
...
var acircle = new shapegenerator(200, 333, 10)
var bcircle = new shapegenerator(333, 222, 60)
var dcircle = new shapegenerator(250,69,42)
...
function draw() {
  ...
  for (var i = 0; i < myXs.length; i++) {
    ...
    // check the boundaries of all the items in the array in the for loop
    // no need for separate checks any longer.  Increase the array and nothing
    // here has to change. Yes! :)
    if (myXs[i] <= 500) {
      myXs[i] += myxspeeds[i];
    } else if (myXs[i] >= 500) {
      myXs[i] = 0;
